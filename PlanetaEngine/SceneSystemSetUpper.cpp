...
namespace planeta_engine {
	namespace core {
		...
		bool SceneSystemSetUpper::operator()(Scene& scene)
		{
			...
			//システムプロセス追加
			//衝突判定プロセス
			auto col_det_proc = p_mgr.AddSystemProcess<system_processes::TCollisionDetect>(p_mgr.GetKeyPosition(gp_syskp::CollisionDetectTaskKeyPositionID));
			...
			auto godp = p_mgr.AddSystemProcess<game_processes::TInstant>(p_mgr.GetKeyPosition(gp_syskp::GameObjectDrawTaskKeyPositionID));
			...
			//ゲームオブジェクト更新プロセス
			auto goup = p_mgr.AddSystemProcess<game_processes::TInstant>(p_mgr.GetKeyPosition(gp_syskp::GameObjectUpdatetTaskKeyPositionID));
			goup->SetExcuteFunction([&game_object_manager = scene.game_object_manager()]{ game_object_manager.Update(); });
			//UI描画プロセス
			auto uidp = p_mgr.AddSystemProcess<game_processes::TInstant>(p_mgr.GetKeyPosition(gp_syskp::GUIDrawTaskKeyPositionID));
			uidp->SetExcuteFunction([&ui_manager = scene.gui_manager(),drawer = scene_data.screen_drawer_ui] {ui_manager.Draw(*drawer); });
			//UI更新プロセス
			auto uiup = p_mgr.AddSystemProcess<game_processes::TInstant>(p_mgr.GetKeyPosition(gp_syskp::GUIUpdateTaskKeyPositionID));
			uiup->SetExcuteFunction([&ui_manager = scene.gui_manager()] {ui_manager.Update(); });
			//Transform関連
			auto tavp = p_mgr.AddSystemProcess<game_processes::TInstant>(p_mgr.GetKeyPosition(gp_syskp::TransformApplyVelocityTaskKeyPositionID));
