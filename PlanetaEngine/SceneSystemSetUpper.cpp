...
namespace planeta_engine {
	namespace core {
		...
		bool SceneSystemSetUpper::operator()(Scene& scene)
		{
			...
			auto godp = p_mgr.AddSystemProcess<game_processes::InstantProcess>(p_mgr.GetKeyPosition(gp_syskp::GameObjectDrawProcessKeyPositionID));
			...
			//ゲームオブジェクト更新プロセス
			auto goup = p_mgr.AddSystemProcess<game_processes::InstantProcess>(p_mgr.GetKeyPosition(gp_syskp::GameObjectUpdatetProcessKeyPositionID));
			goup->SetExcuteFunction([&game_object_manager = scene.game_object_manager()]{ game_object_manager.Update(); });
			//UI描画プロセス
			auto uidp = p_mgr.AddSystemProcess<game_processes::InstantProcess>(p_mgr.GetKeyPosition(gp_syskp::GUIDrawProcessKeyPositionID));
			uidp->SetExcuteFunction([&ui_manager = scene.gui_manager(),drawer = scene_data.screen_drawer_ui] {ui_manager.Draw(*drawer); });
			//UI更新プロセス
			auto uiup = p_mgr.AddSystemProcess<game_processes::InstantProcess>(p_mgr.GetKeyPosition(gp_syskp::GUIUpdateProcessKeyPositionID));
			uiup->SetExcuteFunction([&ui_manager = scene.gui_manager()] {ui_manager.Update(); });
			//Transform関連
			auto tavp = p_mgr.AddSystemProcess<game_processes::InstantProcess>(p_mgr.GetKeyPosition(gp_syskp::TransformApplyVelocityProcessKeyPositionID));
