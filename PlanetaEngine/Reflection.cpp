...
namespace planeta {
	...
	std::shared_ptr<Object> Reflection::CreateObjectByID(const std::string& id)noexcept {
		...
		if (it == id_map.end()) {
			PE_LOG_ERROR("ë∂ç›ÇµÇ»Ç¢Object(ID:", id, ")Ç™éwíËÇ≥ÇÍÇ‹ÇµÇΩÅB");
			return nullptr;
		} else {
			return (it->creator)();
		}
	...
	std::shared_ptr<Object> Reflection::CreateObjectByStdTypeInfo(const std::type_info& t_info)noexcept {
		...
		if (it == std_ti_map.end()) {
			PE_LOG_ERROR("ë∂ç›ÇµÇ»Ç¢Object(Type:", t_info.name(), ")Ç™éwíËÇ≥ÇÍÇ‹ÇµÇΩÅB");
			return nullptr;
		} else {
			return (it->creator)();
		}
	...
	std::string Reflection::GetTypeIDByStdTypeInfo(const std::type_info& tinfo) {
		...
		if (it == std_ti_map.end()) {
			throw reflection_error(util::ConvertAndConnectToString("ìoò^Ç≥ÇÍÇƒÇ¢Ç»Ç¢å^\"", tinfo.name(), "\"Ç™éwíËÇ≥ÇÍÇ‹ÇµÇΩÅB"));
		}
		return it->id;
	}
	...
	const std::type_info& Reflection::GetStdTypeInfoByTypeID(const std::string& id) {
		...
		auto it = id_map.find(id);
		if (it == id_map.end()) {
			throw reflection_error(util::ConvertAndConnectToString("ìoò^Ç≥ÇÍÇƒÇ¢Ç»Ç¢å^ID\"", id, "\"Ç™éwíËÇ≥ÇÍÇ‹ÇµÇΩÅB"));
		}
	...
	void Reflection::RegisterObject_(const std::type_info& tinfo, const std::string& id, const std::function<std::shared_ptr<core::Object>()>& creator) {
		if (!impl_().type_data_map.insert({ tinfo,id,creator }).second) {
			throw reflection_error(util::ConvertAndConnectToString("å^\"", tinfo.name(), "\"Ç™èdï°ìoò^Ç≥ÇÍÇ‹ÇµÇΩÅB(ID:\"", id, "\")"));
		}
	}
}