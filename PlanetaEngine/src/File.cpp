...
namespace planeta {
	...
	File::File(const File& obj) : extension_(obj.extension_), is_available_(obj.is_available_), size_(obj.size_),file_name_(obj.file_name_) {
	...
	File::File(File&& obj) : extension_(std::move(obj.extension_)), is_available_(obj.is_available_), size_(obj.size_), data_top_(obj.data_top_), file_name_(std::move(obj.file_name_)) {
	...
	bool File::WriteData(size_t pos, const unsigned char* data_top, size_t data_size, bool auto_extend /*= false*/) {
		...
		if (size() < pos + data_size) { //自動拡張が有効だったらサイズを増やす
