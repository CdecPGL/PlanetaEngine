...
namespace plnt {
	namespace mp_util {
		...
		struct ApplyToList {
			using list = Cons<typename Proc<typename List::first>::type, typename ApplyToList<
		...
		private:
			template <typename T>
			struct BoostCallTraitsParamTypeWrapper {
		...
		struct ExpandParameterList2 {
			using type = typename ExpandParameterList2<
		...
		std::string ConvertParametersToString(FirstParam fparam, Params... params) {
			return std::move(
				std::string(typeid(FirstParam).name()) + " " + boost::lexical_cast<std::string>(fparam) + "," +
		...
		template <typename FirstParam>
		std::string ConvertParameterTypesToString() { return std::move(std::string(typeid(FirstParam).name())); };
		...
		struct AllOf<UnaryPredicate, First, Rest...> : public std::conditional_t<
				UnaryPredicate<First>::value, AllOf<UnaryPredicate, Rest...>, std::false_type> { };
		...
		template <template<class> class UnaryPredicate, typename First, typename... Rest>
		struct AnyOf<UnaryPredicate, First, Rest...> : public std::conditional_t<
				UnaryPredicate<First>::value, std::true_type, AnyOf<UnaryPredicate, Rest...>> { };

		//! メンバ関数の引数の数を取得する
		template <class C, typename Ret, typename... Args>
		constexpr int GetFunctionArgCount(Ret (C::*)(Args...)) { return sizeof...(Args); }

		//! 関数の引数の数を取得する
		template <class C, typename Ret, typename... Args>
		constexpr int GetFunctionArgCount(Ret (Args...)) { return sizeof...(Args); }
		...
		template <typename T>
		struct IsIStreamCompatible<T, decltype(std::declval<std::istream &>() >> std::declval<T &>(), std::declval<
				                           void>()
		...
		template <typename T>
		struct IsOStreamCompatible<T, decltype(std::declval<std::ostream &>() << std::declval<T &>(), std::declval<
				                           void>()
		                           )> : public std::true_type { };
