...
namespace plnt {
	...
	bool NormalFolderManipulator::OpenProc(const std::string &path) {
		...
		if (boost::filesystem::is_directory(bpath)) { return true; } else {
	...
	bool NormalFolderManipulator::LoadFileProc(const std::string &fn, File &file) { return LoadFileByPath(file, fn); }
	...
	size_t NormalFolderManipulator::GetFileCountProc() const {
		...
		BOOST_FOREACH(const bfs::path& p, std::make_pair(bfs::recursive_directory_iterator(path),
			              bfs::recursive_directory_iterator())) { if (!bfs::is_directory(p)) { ++cnt; } }
		return cnt;
	}
	...
	bool NormalFolderManipulator::CheckFileExistenceProc(const std::string &ppath) const {
		...
		if (!result || error) { return false; } else { return true; }
	}
