...
namespace plnt {
	namespace private_ {
		...
		public:
			...
			WeakPointerMenberFunctionEventHandlerHolder(const WeakPointer<C> &wp,
			                                            FunctionType f) noexcept : w_ptr_(wp), func_(f) { }
		...
		public:
			GarbageEventHandlerHolder(std::unique_ptr<IEventHandlerHolder<EventArgType>> &&garbage) noexcept : garbage_(
				std::move(garbage)) { }
		...
		public:
			GarbageEventHandlerHolder(std::unique_ptr<IEventHandlerHolder<void>> &&garbage) noexcept : garbage_(
				std::move(garbage)) { }
		...
		public:
			...
			DelegateConnecter(const std::weak_ptr<HandlerListType> &handler_list,
			                  size_t id) noexcept : handler_list_(handler_list), id_(id) { }
	...
	*/
	...
	public:
		...
		explicit DelegateConnection(std::unique_ptr<private_::IDelegateConnecter> &&cp) noexcept : connector_(
			std::move(cp)) { }

		~DelegateConnection() noexcept { if (connector_) { connector_->Remove(); } }

		DelegateConnection &operator =(const DelegateConnection &wpdc) = delete;

		DelegateConnection &operator =(DelegateConnection &&wpdc) noexcept {
			connector_ = std::move(wpdc.connector_);
			return *this;
		}
	...
	*/
	...
	public:
		...
		Delegate() noexcept: handlers_(
			std::make_shared<private_::DelegateConnecter<EventArgType>::HandlerListType>()) { }
		...
		void operator()(HandlerParamType e) {
			for (auto it = handlers_->begin(); it != handlers_->end();) {
				if (it->second->Call(e)) { ++it; } else { handlers_->erase(it++); }
			}
		}
		...
		*/
		...
		DelegateConnection Add(const WeakPointer<C> &c, typename void (C::*f)(HandlerParamType)) {
			...
			handlers_->emplace(
				id, std::make_unique<private_::WeakPointerMenberFunctionEventHandlerHolder<EventArgType, C>>(c, f));
	...
	public:
		...
		void operator()() {
			for (auto it = handlers_->begin(); it != handlers_->end();) {
				if (it->second->Call()) { ++it; } else { handlers_->erase(it++); }
			}
		}
		...
		*/
		...
		DelegateConnection Add(const WeakPointer<C> &c, typename void (C::*f)()) {
			size_t id = id_counter_++;
			handlers_->emplace(
				id, std::make_unique<private_::WeakPointerMenberFunctionEventHandlerHolder<void, C>>(c, f));
