...
namespace plnt {
	namespace private_ {
		...
		std::pair<int, int> StandardCollisionWorld::ProcessCollisionBetweenTwoGroups(
			CollisionGroupType &group1, CollisionGroupType &group2, CollisionEventQue &collision_event_que) const {
			int collision_process_count{0}, collision_count{0};
		...
		std::pair<int, int> StandardCollisionWorld::ProcessCollisionWithGround(
			CollisionEventQue &collision_event_que) const {
			int collision_process_count{0}, collision_count{0};
			for (const auto &col_com : collision_with_ground_list_) {
				++collision_process_count;
				auto &col_reg_data = col_com.get();
				if (col_reg_data.collider2d_data.collider2d.DetectCollision(
		...
		bool StandardCollisionWorld::change_collision_group(const CCollider2D *col_com_ptr,
			...
			if (group_it == collision_groupes_.end()) {
				//指定されたグループが存在しない
		...
		void StandardCollisionWorld::RemoveAll() {
			for (auto &group : collision_groupes_) { group.second.clear(); }
