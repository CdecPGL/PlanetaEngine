...
namespace plnt {
	GOComponentGetter::GOComponentGetter(const private_::GameObjectComponentHolder &com_holder) : com_holder_(
		com_holder) { }

	std::vector<std::shared_ptr<GameObjectComponent>> GOComponentGetter::GetAllComponentsByTypeInfo(
	...
	std::shared_ptr<GameObjectComponent> GOComponentGetter::GetComponentByTypeInfo_(
		const std::type_info &ti, const std::function<bool(GameObjectComponent *goc)> &type_checker) const {
		auto com = com_holder_.GetComponentByTypeInfo(ti, type_checker);
		if (com) { return com; } else {
