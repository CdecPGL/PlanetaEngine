...
namespace plnt {
	...
	*/
	...
	public:
		...
		//! コンポーネントを型で取得する。
		template <class ComT>
		weak_pointer<ComT> get_component() const {
		...
		//! コンポーネントを型で全て取得する。
		template <class ComT>
		std::vector<weak_pointer<ComT>> get_all_components() const {
			static_assert(std::is_base_of_v<game_object_component, ComT> == true,
			              "ComT must drive GameObjectComponent.");
			...
			std::vector<weak_pointer<ComT>> out;
		...
		template <class ComT>
		std::vector<weak_pointer<ComT>> get_all_components_with_condition(
