...
namespace plnt {
	...
	class WeakPointer {
		template <typename T0, typename T1>
		friend WeakPointer<T0> static_weak_pointer_cast(const WeakPointer<T1> &ptr);
		template <typename T0, typename T1>
		friend WeakPointer<T0> dynamic_weak_pointer_cast(const WeakPointer<T1> &ptr);
	...
	public:
		...
		WeakPointer(const WeakPointer<T1> &wp) : w_ptr_(wp.w_ptr_) {
			static_assert(std::is_base_of<T, T1>::value == true, "T is not base of T1.");
		}

		template <typename T1>
		WeakPointer(const std::weak_ptr<T1> &wp) : w_ptr_(wp) {
			static_assert(std::is_base_of<T, T1>::value == true, "T is not base of T1.");
		}

		template <typename T1>
		WeakPointer(const std::shared_ptr<T1> &sp) : w_ptr_(sp) {
			static_assert(std::is_base_of<T, T1>::value == true, "T is not base of T1.");
		}
		...
		bool operator==(bool b) const { return operator bool() == b; }
	...
	template <typename T>
	WeakPointer<T> MakeWeak(const std::shared_ptr<T> &s_ptr) { return WeakPointer<T>(s_ptr); }
}
