...
namespace plnt {
	namespace private_ {
		namespace collision_detect_functions {
			bool CollisionDetectCircleAndCirecle(CCircleCollider2D &circle_collider0,
			                                     CCircleCollider2D &circle_collider1) {
				...
				if ((pos1 - pos0).length_square() <=
					std::pow(circle_radius0 + circle_radius1, 2)) { return true; } else { return false; }
			}

			bool CollisionDetectCircleAndStraightLine(CCircleCollider2D &circle_collider,
			                                          CStraightLineCollider2D &straightline_collider) {
				...
				auto line_vector = Vector2Dd(std::cos(straightline_collider.GetCollisionGlobalRotationRad()),
				                             std::sin(straightline_collider.GetCollisionGlobalRotationRad())) *
					straightline_collider.GetCollisionScale() * straightline_collider.length();
				...
				if (distance > circle_radius) { return false; } else {
					//円が線分の両端外側にあるか
					...
					if (line_start_to_circle_center_dot_line_vector * Dot(line_end_to_circle_center, line_vector) <=
						0) {
						//線分内にある
						return true;
					} else {
						...
						if (line_start_to_circle_center_dot_line_vector > 0) {
							//線分の終点側にある
							if (line_end_to_circle_center.length() <= circle_radius) { return true; } else {
								return false;
							}
						} else {
							//線分の始点側にある
							if (line_start_to_circle_center.length() <= circle_radius) { return true; } else {
								return false;
							}
						}
					}
