	...
	namespace private_ {
		...
		public:
			...
			bool DisposeTaskRequest(TaskData &tdata, bool is_system_task) {
				...
				switch (tdata.state) {
					...
					case TaskState::PauseRequested:
						management_process_list_.push_back([&tdata, this] { DisposeTask(tdata); });
						break;
					default:
						assert(false);
						return false;
			...
			bool PauseTaskRequest(TaskData &tdata) {
				...
				//停止処理を追加
				management_process_list_.push_back([&tdata, this] { InctivateTask(tdata); });
				tdata.state = TaskState::PauseRequested;
				return true;
			}
			...
			void AllClear() {
				...
				for (auto &&tg : task_excution_list_) { tg.clear(); }
			}
