...
namespace plnt {
	namespace private_ {
		namespace {
			...
			/*DX形式頂点情報を設定*/
			inline void SetPEVertex2DToDXVERTEX3D(VERTEX3D &dx_vtx3d, const util::Vertex2D &pe_vtx2d,
			...
			/*DX形式ポリゴンインデックス情報を設定*/
			inline void ConvertPEPolygonIndexToDXIndexArray(unsigned short *dx_idx_array_ptr, size_t write_pos_top,
			                                                const std::array<size_t, 3> &pe_pidx) {
				for (int j = 0; j < 3; ++j) { dx_idx_array_ptr[write_pos_top + j] = (unsigned short)pe_pidx[j]; }
			}
		}
		...
		void GraphDrawData2D::SetGraphResource(const std::shared_ptr<RGraph> &g_res) {
			...
			if (dx_data_->vertexes) {
				for (size_t i = 0; i < dx_data_->vertex_count; ++i) {
					...
					SetUVToDXVERTEX3D(vtx, vertex_uv_information_buffer_[i].x, vertex_uv_information_buffer_[i].y,
					                  g_res->image_area());
				}
			}
		}
		...
		void GraphDrawData2D::SetVertexCount(size_t c) {
			...
			for (size_t i = 0; i < c; ++i) { InitializeDXVERTEX3D(dx_data_->vertexes[i]); }
		...
		void GraphDrawData2D::SetVertexesAndPolygonIndexes(const std::vector<util::Vertex2D> &vertexes,
		                                                   const std::vector<std::array<size_t, 3>> &indexes) {
		...
		size_t GraphDrawData2D::vertex_count() const { return dx_data_->vertex_count; }

		size_t GraphDrawData2D::polygon_count() const { return dx_data_->polygon_count; }
