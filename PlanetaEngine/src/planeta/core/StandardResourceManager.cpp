...
namespace plnt {
	namespace private_ {
		...
		bool StandardResourceManager::LoadResourceList_() {
			...
			//リソースの登録
			for (auto &&pair : full_id_to_resource_data_map) { RegisterResourceData_(std::move(*pair.second)); }
		...
		bool StandardResourceManager::SetNotUnloadTags_(const std::set<std::string> &tags) {
			...
			std::set_difference(tags.begin(), tags.end(), not_unload_tags_.begin(), not_unload_tags_.end(),
			                    std::back_inserter(new_set_tags));
			//指定から外れたタグを求める
			std::vector<std::string> new_unset_tags;
			std::set_difference(not_unload_tags_.begin(), not_unload_tags_.end(), tags.begin(), tags.end(),
		...
		std::string StandardResourceManager::GetFullIDFromTypeAndID_(const boost::typeindex::type_index &type_index,
		                                                             const std::string &id) {
