...
namespace plnt {
	...
	FileStreamBuf::int_type FileStreamBuf::underflow() { return std::char_traits<char>::eof(); }

	FileStreamBuf::int_type FileStreamBuf::uflow() { return std::char_traits<char>::eof(); }
	...
	FileStreamBuf::pos_type FileStreamBuf::seekoff(off_type off, std::ios_base::seekdir dir,
		...
		switch (mode & (std::ios_base::in | std::ios_base::out)) {
			...
			case std::ios_base::in: {
				...
				cur_pos += way + off;
				if (cur_pos < 0 || file_.size() >= cur_pos) { return std::char_traits<char>::eof(); } else {
	...
	bool FileStreamBuf::SaveDataFromWriteBuffer(size_t block, pos_type buf_off) {
		//バッファをファイルに書き込み
		if (file_.WriteData(block * buffer_size, reinterpret_cast<unsigned char *>(pptr()), pptr() - pbase(), true) ==
			false) { return false; }
		//バッファクリア
