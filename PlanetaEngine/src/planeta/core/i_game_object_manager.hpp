...
namespace plnt {
	...
	public:
		...
		[[nodiscard]] virtual WeakPointer<i_game_object> create_game_object(const std::string &game_object_def_file_id) = 0;
		/*定義ファイルを指定して名前付きゲームオブジェクトを作成*/
		[[nodiscard]] virtual WeakPointer<i_game_object> create_game_object(const std::string &game_object_def_file_id,
		                                                    const std::string &name) = 0;
		/*コンポーネントタイプIDを指定してゲームオブジェクトを作成*/
		[[nodiscard]] virtual WeakPointer<i_game_object> create_game_object_with_component_type_id_list(
