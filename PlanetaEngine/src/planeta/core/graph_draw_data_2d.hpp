...
namespace plnt {
	...
	namespace private_ {
		...
		public:
			...
			bool set_vertex(size_t idx, const util::Vertex2D &vtx); //頂点を設定する
			// NOLINTNEXTLINE(modernize-use-nodiscard)
			bool set_vertex_position(size_t idx, const Vector2Df &pos) const; //頂点の位置を設定する
			bool set_vertex_uv(size_t idx, const Vector2Df &uv); //頂点のUV座標を設定する
			// NOLINTNEXTLINE(modernize-use-nodiscard)
			...
			void set_vertexes(const std::vector<util::Vertex2D> &vertexes); //頂点を全て設定
			void set_polygon_indexes(const std::vector<std::array<size_t, 3>> &polygon_indexes) const; //ポリゴンインデックスを全て設定
			void set_vertexes_and_polygon_indexes(const std::vector<util::Vertex2D> &vertexes,
