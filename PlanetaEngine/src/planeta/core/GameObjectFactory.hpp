...
namespace plnt {
	namespace private_ {
		...
		public:
			...
			std::shared_ptr<GameObjectBase> GetNewGameObject(const std::string &game_object_resource_id,
			...
			/// <param name="scene_data">対象のシーンデータ</param>
			/// <returns>新しいゲームオブジェクト</returns>
			std::shared_ptr<GameObjectBase> GetNewGameObject(
		...
		private:
			...
			std::shared_ptr<GameObjectBase> CreateGameObjectFromComponentTypeList_(
				const std::vector<std::string> &game_object_component_type_id_list,
				const WeakPointer<private_::ISceneInternal> &scene_data);
			...
			/// <param name="scene_data">作成先のシーンデータ</param>
			/// <returns>作成されたゲームオブジェクト</returns>
			std::shared_ptr<GameObjectBase> CreateGameObjectFromResource_(
			...
			/// <param name="scene_data">作成先のシーンデータ</param>
			/// <returns>作成されたゲームオブジェクト</returns>
			std::shared_ptr<GameObjectBase> CloneGameObjectFromTemplate_(
				const std::shared_ptr<GameObjectBase> &go_temp,
				const WeakPointer<private_::ISceneInternal> &scene_data);
		};
	}
}
