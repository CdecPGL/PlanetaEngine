...
namespace plnt {
	...
	public:
		...
		SignalConnection ConnectFunction(const std::shared_ptr<InsType> &ins, const FuncType &func) {
			return SignalConnection{
				std::move(MySignal::connect(
					SlotType(std::function<SigType>(func), boost::placeholders::_1).track_foreign(ins)))
			};
		}
		...
		SignalConnection ConnectFunction(const WeakPointer<InsType> &ins, const FuncType &func) {
			return SignalConnection{
				std::move(MySignal::connect(
					SlotType(std::function<SigType>(func), boost::placeholders::_1).track_foreign(ins.get_shared())))
			};
		}
		...
		ConnectMemberFunction(const std::shared_ptr<InsType> &ins, RetType (InsType::*m_func)(ArgType)) {
			return SignalConnection{
				std::move(MySignal::connect(SlotType(m_func, ins.get(), boost::placeholders::_1).track_foreign(ins)))
			};
		}
		...
		*/
		...
		SignalConnection ConnectMemberFunction(const WeakPointer<InsType> &ins, RetType (InsType::*m_func)(ArgType)) {
			return SignalConnection{
				std::move(MySignal::connect(
					SlotType(m_func, ins.get(), boost::placeholders::_1).track_foreign(ins.get_shared())))
			};
		}
	...
	public:
		...
		SignalConnection ConnectFunction(const std::shared_ptr<InsType> &ins, const FuncType &func) {
			return SignalConnection{
				std::move(MySignal::connect(SlotType(std::function<RetType(void)>(func)).track_foreign(ins)))
			};
		}
		...
		SignalConnection ConnectFunction(const WeakPointer<InsType> &ins, const FuncType &func) {
			return SignalConnection{
				std::move(MySignal::connect(
					SlotType(std::function<RetType(void)>(func)).track_foreign(ins.get_shared())))
			};
		}
		...
		SignalConnection ConnectMemberFunction(const WeakPointer<InsType> &ins, RetType (InsType::*m_func)()) {
			return SignalConnection{
				std::move(MySignal::connect(SlotType(m_func, ins.get()).track_foreign(ins.get_shared())))
			};
		}
