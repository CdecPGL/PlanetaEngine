...
namespace plnt {
	...
	public:
		...
		explicit constexpr Vector4D(const Vector4D<T2> &obj) : x(static_cast<T>(obj.x)), y(static_cast<T>(obj.y)),
		...
		//キャスト演算子
		template <typename T2>
		explicit constexpr operator Vector4D<T2>() const {
			return Vector4D<T2>(static_cast<T2>(x), static_cast<T2>(y), static_cast<T2>(z), static_cast<T2>(w));
		}
		...
		Vector4D<T> operator/(U num) const {
			...
			Vector4D<T> out(static_cast<T>(static_cast<double>(x) / num), static_cast<T>(static_cast<double>(y) / num),
		...
		//符号反転演算子
		constexpr Vector4D<T> operator-() const { return Vector4D<T>(-x, -y, -z, -w); }
		...
		constexpr bool operator!=(const Vector4D<T> &in) const { return !(*this == in); }
		...
		constexpr double length_square() const { return x * x + y * y + z * z + w * w; }
...
namespace plnt::reflection {
	...
	template <typename T>
	struct reflective_ptree_converter_impl<Vector4D<T>> {
		void operator()(Vector4D<T> &dst, const boost::property_tree::ptree &src) {
			if (src.size() != 4) {
			...
			for (auto &&pp : src) {
				if (pp.first.empty() == false) {
					throw reflection_error(
						util::ConvertAndConnectToString("Vector4DのPtreeキーは空である必要があります。(読み取られたキー:", pp.first, ")"));
				}
				T dat{};
