	...
	namespace private_ {
		...
		*/
		...
		public:
			...
			//タイプでコンポーネントを取得
			std::shared_ptr<GameObjectComponent> GetComponentByTypeInfo(const std::type_info &ti,
			                                                            const std::function<bool(GameObjectComponent *)>
			                                                            &
			                                                            type_checker) const;
			//コンポーネントを型で取得する。
			template <class ComT>
			std::shared_ptr<ComT> GetComponent() {
				...
				auto ptr = GetComponentByTypeInfo(typeid(ComT), [](GameObjectComponent *goc) {
			...
			std::vector<std::shared_ptr<GameObjectComponent>> GetAllComponentsByTypeInfo(
			...
			std::vector<std::shared_ptr<ComT>> GetAllComponents() {
				static_assert(std::is_base_of<GameObjectComponent, ComT>::value == true,
				              "ComT must derive GameComponent.");
				auto go_list = std::move(GetAllComponentsByTypeInfo(typeid(ComT), [](GameObjectComponent *com) {
