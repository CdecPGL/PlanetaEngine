...
namespace plnt {
	...
	public:
		...
		void DrawPolygon(const std::vector<Vector2Di>& positions, const std::vector<std::array<int, 3>>& indexes, const plnt::Color& color);
		/*! 文字列を描画(描画位置、拡大度、描画文字列、色、縁色、フォント定義リソース)*/
		void DrawString(const Vector2Di& position, const Vector2Dd scale, const std::string& str, const plnt::Color& color, const plnt::Color& outline_color, const std::shared_ptr<RFont>& font_resource);
		/*! 文字列を描画(描画位置、拡大度、描画文字列、色、縁色)*/
		void DrawStringByDefaultFont(const Vector2Di& position, const Vector2Dd scale, const std::string& str, const plnt::Color& color, const plnt::Color& outline_color);
	private:
