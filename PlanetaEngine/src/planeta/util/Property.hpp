...
namespace plnt {
	namespace util {
		...
		public:
			...
			//プロパティ型への変換
			operator PropertyType() const { return (instance_.*getter)(); }

			/*()を用いたアクセス*/
			GetterRet operator()() const & { return get(); }
			...
			auto operator+(const ROP &right_op) const { return get() + right_op; }
		...
		public:
			...
			/*()を用いたアクセス*/
			void operator()(SetterArg v) { set(v); }
		...
		/*読み書きプロパティ*/
		template <class C, typename PropertyType, typename GetterRet, typename SetterArg, GetterRet(C::*getter)() const,
		          void (C::*setter)(SetterArg)>
		...
		template <class C, typename PropertyType, const PropertyType&(C::*getter)() const, void(C::*setter)(
			          const PropertyType &)>
		using ReadWritePropertyR = ReadWriteProperty<
			C, PropertyType, const PropertyType &, const PropertyType &, getter, setter>;
