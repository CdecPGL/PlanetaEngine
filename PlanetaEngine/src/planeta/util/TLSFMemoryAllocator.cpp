...
namespace plnt {
	namespace util {
		...
		public:
			_Impl(TLSFMemoryAllocator &ptlsf) : _tlsf(ptlsf),
			                                    _second_level_index_separation_exponent_of_2(
				                                    SECOND_LEVEL_INDEX_SEPARATION_EXPONENT_OF_TWO),
			                                    _second_level_index_separation(
				                                    1u << _second_level_index_separation_exponent_of_2),
			                                    _free_block_list(nullptr), _free_block_list_size(0) { }
		...
		private:
			...
			public:
				...
				size_t *GetFooterPointer() {
					return reinterpret_cast<size_t *>(reinterpret_cast<char *>(GetDataBlockPointer()) + _data_size);
				}

				bool ErrorCheck() const { return _error_check_data != error_check_code; } //エラーチェック。エラーならtrue
				bool FooterErrorCheck() const {
					return GetTotalSize() != *const_cast<_MemoryBlock *>(this)->GetFooterPointer();
				} //フッターの値が正当化確認
				...
				static size_t CalcTotalSizeByDataSize(size_t data_size) { return data_size + manage_area_size; }
				//データサイズから、ブロックの号掲載ぞを計算する
			...
			_MemoryBlock *_marge_free_block(_MemoryBlock *);
			//可能ならフローブロックを結合する(この関数内ではフリーブロックリストへの登録は行わない。マージしたブロックの登録解除は行う)
			_MemoryBlock *_devide_free_block(_MemoryBlock *, size_t);
			//可能ならフリーブロックを指定サイズに分割する。分割した右ブロックへのポインタを返す(この関数内ではフリーブロックリストへの登録は行わない)
		...
		inline bool TLSFMemoryAllocator::_Impl::reserve_memory(size_t size) {
			...
			_MemoryBlock *foot_dumy = new(
					reinterpret_cast<char *>(_tlsf._memory_pool) + _tlsf._memory_pool_size -
					_MemoryBlock::manage_area_size)
				_MemoryBlock();
			...
			//領域に収まるようにメモリブロック作成
			size_t create_block_size = (_tlsf._memory_pool_size - head_dumy->GetTotalSize() - foot_dumy->
				GetTotalSize());
			_MemoryBlock *block = new(reinterpret_cast<char *>(head_dumy) + head_dumy->GetTotalSize())_MemoryBlock(
				create_block_size, true);
			//作製したデータの格納カテゴリを求める
			size_t cate1 = calcMSB(block->GetDataSize() + 1);
			size_t cate2 = _calc_second_level_index(block->GetDataSize() + 1, cate1);
			if (cate2 == 0) {
				--cate1;
				cate2 = _second_level_index_separation - 1;
			} else { --cate2; }
		...
		inline TLSFMemoryAllocator::MemoryDetailScanResult TLSFMemoryAllocator::_Impl::detail_scan() const {
			...
			while (ptr < _tlsf._memory_pool + _tlsf._memory_pool_size) {
				...
				if (block->ErrorCheck() || block->FooterErrorCheck()) {
					res.corruption = true;
					break;
				} //エラーチェック
				...
				if (use_flag) {
					//アクティブブロック
					if (data_size > 0) { ++res.active_block_count; } else { ++res.dumy_block_count; }
					res.allocated_memory_size += data_size;
					res.using_memory_size += total_size;
				} else {
					//フリーブロック
					if (data_size > 0) { ++res.free_block_count; } else { ++res.dumy_block_count; }
					res.available_memory_size += data_size;
					res.free_memory_size += total_size;
					if (res.available_max_memory_allocation_size < data_size) {
						res.available_max_memory_allocation_size = data_size;
					}
				}
			...
			for (size_t i = 0; i < _free_block_list_size; ++i) {
				...
				while (list_top == list_top->next()) {
					++free_list_count;
					is_exist_free_block = true;
				}
				if ((((_second_level_index_free_bit_array[i / _second_level_index_separation]) & (1u << (i %
					     _second_level_index_separation)))
					     ? true
					     : false) != (is_exist_free_block ? true : false)) {
					res.inconsistency_at_second_free_block_list_bit_array = true;
				}
				if (is_exist_free_block) {
					correct_first_free_block_bit_array |= (1u << (i / _second_level_index_separation));
				}
			}
			//エラー検出
			...
			res.inconsistency_at_second_free_block_list_bit_array = (_first_level_index_free_bit_array !=
				correct_first_free_block_bit_array);
			res.error = res.corruption || res.inconsistency_at_allocated_memory_count || res.
				inconsistency_at_allocated_memory_size || res.inconsistency_at_using_memory_size || res.
				inconsistency_at_dumy_count || res.inconsistency_at_free_block_list || res.
				inconsistency_at_first_free_block_list_bit_array || res.
				inconsistency_at_second_free_block_list_bit_array;
			//断片化率計算
		...
		inline void TLSFMemoryAllocator::_Impl::dealloc(void *ptr) {
			...
			if (ptr < (_tlsf._memory_pool + _MemoryBlock::manage_area_size) || ptr >= (_tlsf._memory_pool + _tlsf.
				_memory_pool_size - _MemoryBlock::manage_area_size)) {
				throw BadAlloc("Designated pointer is out of range,");
			}
			_MemoryBlock *_block = reinterpret_cast<_MemoryBlock *>(reinterpret_cast<char *>(ptr) - sizeof(
				_MemoryBlock));
		...
		inline void TLSFMemoryAllocator::_Impl::release_free_block_list() {
			if (_free_block_list) {
				/*delete[] _free_block_list;*/
				free(_free_block_list);
			}
		...
		/////////////////////////////////////////////////////////////////////////
		inline TLSFMemoryAllocator::_Impl::_MemoryBlock *TLSFMemoryAllocator::_Impl::_pop_free_block(
			size_t cate_1, size_t cate_2) {
			...
			if (free_list_idx >= _free_block_list_size) {
				throw std::logic_error("attempt to access out of free bit list! ");
			}
		...
		inline void TLSFMemoryAllocator::_Impl::_remove_free_block(_MemoryBlock *block) {
			...
			if (_free_block_list[idx].next() == nullptr) {
				_second_level_index_free_bit_array[cate1] &= ~(1u << cate2);
				if (_second_level_index_free_bit_array[cate1] == 0) {
					_first_level_index_free_bit_array &= ~(1u << cate1);
				}
			}
		}
		...
		inline TLSFMemoryAllocator::_Impl::_MemoryBlock *TLSFMemoryAllocator::_Impl::_marge_free_block(
			_MemoryBlock *block) {
			//右
			_MemoryBlock *right = reinterpret_cast<_MemoryBlock *>(reinterpret_cast<char *>(block) + block->
				GetTotalSize());
			if (!right->IsUsing()) {
		...
		inline TLSFMemoryAllocator::_Impl::_MemoryBlock *TLSFMemoryAllocator::_Impl::_devide_free_block(
			_MemoryBlock *block, size_t size) {
			//分割後の残り領域サイズが最低値より小さかったら分割しない確保できないなら
			if (block->GetDataSize() <= size + sizeof(_MemoryBlock) + _MemoryBlock::footer_size +
				_second_level_index_separation) { return nullptr; }
		...
		inline TLSFMemoryAllocator::_Impl::_MemoryBlock::_MemoryBlock(size_t data_size) : _data_size(data_size),
			_use_flag(false), _prev(nullptr), _next(nullptr),
			_error_check_data(error_check_code) { *GetFooterPointer() = GetTotalSize(); }

		inline TLSFMemoryAllocator::_Impl::_MemoryBlock::_MemoryBlock(size_t size, bool fit) : _MemoryBlock(
			fit ? size - sizeof(_MemoryBlock) - footer_size : size) { }

		inline TLSFMemoryAllocator::_Impl::_MemoryBlock::_MemoryBlock(bool list_top_flag) : _data_size(0),
			_use_flag(false), _prev(nullptr), _next(nullptr), _error_check_data(error_check_code) {
			//フッターに合計サイズ書き込み
		...
		/////////////////////////////////////////////////////////////////////////
		TLSFMemoryAllocator::TLSFMemoryAllocator() : _memory_pool_size(0), _memory_pool(nullptr), _using_memory_size(0),
		                                             _allocated_memory_count(0), _impl(new _Impl(*this)) { }

		TLSFMemoryAllocator::TLSFMemoryAllocator(size_t t) : TLSFMemoryAllocator() { ReserveMemory(t); }
		...
		TLSFMemoryAllocator::MemoryDetailScanResult TLSFMemoryAllocator::DetailScan() const {
			return _impl->detail_scan();
		}
