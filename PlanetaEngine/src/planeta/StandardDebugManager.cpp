...
namespace plnt {
	namespace private_ {
		...
		public:
			StandardDebugInformationAdder(ScreenDrawerGUI& screen_drawer_gui) :screen_drawer_gui_(screen_drawer_gui) {};
			virtual void AddLine(const std::string line) override {
				screen_drawer_gui_.DrawStringByDefaultFont({ 0,16 * line_count_ }, { 1,1 }, line, Color::White(), Color::Black());
				++line_count_;
			}
		private:
		...
		void StandardDebugManager::PreRenderingUpdate() {
			...
			if (impl_->is_debug_information_showing) {
				//F2でデバッグ表示する情報切り替え
				if (inp_mgr.KeyPush(Key::F2)) {
					impl_->SwitchShowingDebugInformation();
				}
		...
		bool StandardDebugManager::DeleteDebugInformationChannel(const std::string& channel_id) {
			...
			if (impl_->showing_debug_information_iterator == it) {
				impl_->SwitchShowingDebugInformation();
			}
