#include"planeta/InputUtility.hpp"
...
namespace plnt {
	namespace private_ {
		namespace {
			...
			void setKeyNameMap() {
				for (const auto& p : _key_name_map) {
					_key_name_mapr.insert(std::make_pair(p.second, p.first));
				}
				_knm_setted = true;
			}
			void setPadNameMap() {
				for (const auto& p : _pad_name_map) {
					_pad_name_mapr.insert(std::make_pair(p.second, p.first));
				}
				_pnm_setted = true;
			}
			void setButtonNameMap() {
				for (const auto& p : _button_name_map) {
					_button_name_mapr.insert(std::make_pair(p.second, p.first));
				}
				_bnm_setted = true;
			}
		}
