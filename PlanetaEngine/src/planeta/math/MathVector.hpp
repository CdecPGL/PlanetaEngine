...
namespace plnt {
	namespace math {
		...
		class MathVector : public VectorElementIdentifer<T> {
			...
			static_assert(S >= VectorElementIdentiferType::minimum_vector_size,
			              "The vector size must not to be less than VectorElementIdentifer::minimum_vector_size.");
		...
		public:
			...
			template <typename T2, int S2, template<typename>class VEI>
			bool operator!=(const MathVector<T2, S2, VEI> &v) const { return !(*this == v); }
			...
			const MathVector<T, S> &operator+=(const MathVector<T2, S2, VEI> &v) {
				constexpr int MinS = S < S2 ? S : S2;
				for (int i = 0; i < MinS; ++i) { (*this)[i] += v[i]; }
				return *this;
			}
			...
			auto operator+(const MathVector<T2, S2, VEI> &v) const {
				...
				constexpr int MinS = S < S2 ? S : S2;
				for (int i = 0; i < MinS; ++i) { ret[i] += v[i]; }
				return std::move(ret);
			}
		...
		auto operator*(const SCALA_T &s, const MathVector<VECTOR_T, VECTOR_S, VEI> &v) { return v * s; }
	}
}
