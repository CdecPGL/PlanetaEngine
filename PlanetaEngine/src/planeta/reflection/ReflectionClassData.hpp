...
namespace plnt::reflection {
	namespace core {
		...
		public:
			...
			/*ReadWrite*/
			PropertyData1(GetterType getter, SetterType setter) : member_variable_pointer_(mvp)
			:
			getter_ (getter), setter_(setter) { }

			/*ReadOnly*/
			PropertyData1(GetterType getter) : member_variable_pointer_(mvp)
			:
			getter_ (getter), setter_(nullptr) { }

			/*WriteOnly*/
			PropertyData1(SetterType setter) : member_variable_pointer_(mvp)
			:
			getter_ (
		...
		public:
			...
			/*ReadWrite*/
			PropertyData2(GetterType getter, SetterType setter) : member_variable_pointer_(mvp)
			:
			getter_ (getter), setter_(setter) { }

			/*ReadOnly*/
			PropertyData2(GetterType getter) : member_variable_pointer_(mvp)
			:
			getter_ (getter), setter_(nullptr) { }

			/*WriteOnly*/
			PropertyData2(SetterType setter) : member_variable_pointer_(mvp)
			:
			getter_ (
		...
		public:
			...
			ClassDataResister &Variable(const std::string &identifer, T C::*member_veriable_pointer) {
				class_data_->variable_.emplace(
					identifer, std::make_unique<MemberVariableData<C, T>>(member_veriable_pointer));
				return *this;
			}
			...
			ClassDataResister &Function(const std::string &identifer, Ret (C::*member_function_pointer)(Args...)) {
				class_data_->function_.emplace(
					identifer, std::make_unique<MemberFunctionData<C, Ret, Args...>>(member_function_pointer));
				return *this;
			}
