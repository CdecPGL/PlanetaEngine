...
namespace plnt::reflection {
	...
	*/
	...
	public:
		...
		*/
		...
		template <typename GRet, typename SRet, typename SArg>
		ClassRegisterer &Property(const std::string &prop_id, GRet (C::*getter)() const,
		...
		*/
		...
		ClassRegisterer &ShallowCopyTarget(V C::*var) noexcept {
			...
			class_info_.copy_handler_list.push_back([var](const Reflectable &src, Reflectable &dst) {
				ReflectiveCopyHandler(static_cast<C &>(dst).*var, static_cast<const C &>(src).*var);
			});
			return *this;
		}
	...
	ClassRegisterer<C> &ClassRegisterer<C>::Variable(const std::string &var_id, V C::*mv_ptr) noexcept {
		...
		vpinfo.getter = [mv_ptr](Reflectable &c) { return std::move(boost::any(static_cast<C &>(c).*mv_ptr)); };
	...
	*/
	template <class C>
	template <typename GRet, typename SRet, typename SArg>
	ClassRegisterer<C> &ClassRegisterer<C>::Property(const std::string &prop_id, GRet (C::*getter)(),
	                                                 SRet (C::*setter)(SArg)) noexcept {
	...
	template <class C>
	template <typename GRet, typename SRet, typename SArg>
	ClassRegisterer<C> &ClassRegisterer<C>::Property(const std::string &prop_id, GRet (C::*getter)() const,
	                                                 SRet (C::*setter)(SArg)) noexcept {
		using PTypeS = std::remove_cv_t<std::remove_reference_t<SArg>>;
		using PTypeG = std::remove_cv_t<std::remove_reference_t<GRet>>;
		static_assert(std::is_same<PTypeS, PTypeG>::value, "The type of getter or setter is incorrect.");
	...
	template <class C>
	template <typename PType, typename Getter, typename Setter>
	ClassRegisterer<C> &ClassRegisterer<C>::Property_(const std::string &prop_id, const Getter &getter,
		...
		pinfo.getter = [getter](Reflectable &c) { return boost::any((static_cast<C &>(c).*getter)()); };
	...
	*/
	template <class C>
	template <typename Getter>
	ClassRegisterer<C> &ClassRegisterer<
		C>::ReadOnlyProperty(const std::string &prop_id, const Getter &getter) noexcept {
	...
	*/
	...
	template <typename Setter>
	ClassRegisterer<C> &ClassRegisterer<
		C>::WriteOnlyProperty(const std::string &prop_id, const Setter &setter) noexcept {
		using PType = std::remove_cv_t<std::remove_reference_t<boost::mpl::at_c<
