...
namespace planeta {
	void ScreenDrawerGUI::DrawGraph(const Vector2Di& position, const Vector2Di& size, const Vector2Dd& center_pivot, double rotation_rad, const RectAnglei& draw_area_on_graph, bool reverse, const std::shared_ptr<RGraph>& graph_resource) {
		screen_.ReserveDraw([position,size,draw_area_on_graph,reverse,graph_resource,rotation_rad,center_pivot]() {
			...
			double scale_x = (double)size.x / draw_area_on_graph.size.x;
			double scale_y = (double)size.y / draw_area_on_graph.size.y;
			SetDrawBlendMode(DX_BLENDMODE_NOBLEND, 255);
			DrawRectRotaGraph3(
				position.x, position.y,
				draw_area_on_graph.position.x, draw_area_on_graph.position.y,
				size.x, size.y,
				static_cast<int>(draw_area_on_graph.size.x * center_pivot.x), 
				static_cast<int>(draw_area_on_graph.size.y  * center_pivot.y),
				scale_x, scale_y,
