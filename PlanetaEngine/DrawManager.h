...
namespace planeta_engine {
	...
	namespace core {
		...
		public:
			...
			void DrawWire(const std::vector<math::Vector2Dd>& positions, double width, const Color& color);
			/*多角形の描画(点座標、三角形ポリゴンインデックス、色)*/
			void DrawPolygon(const std::vector<math::Vector2Dd>& positions, const std::vector<std::array<int, 3>>& indexes, const Color& color);
			//カメラ関数
			/*カメラ位置を設定*/
			void SetCameraPosition(const math::Vector2Dd& position);
			/*カメラ回転度を設定*/
			...
			void DrawUIWire(const std::vector<math::Vector2Di>& positions, int width, const Color& color);
			/*UI多角形を描画(点座標、三角形ポリゴンインデックス、色)*/
			void DrawUIPolygon(const std::vector<math::Vector2Di>& positions, const std::vector<std::array<int, 3>>& indexes, const Color& color);
			/*文字列を描画(描画位置、拡大度、描画文字列、色、縁色、フォント定義リソース)*/
			void DrawUIString(const math::Vector2Di& position, const math::Vector2Dd scale, const std::string& str, const core::Color& color, const core::Color& outline_color, const std::shared_ptr<resources::FontDefinitionResource>& font_definition_resource);
		...
		private:
			DrawManager() = default;
			~DrawManager() = default;
			static math::Vector2Dd _ConvertPosition(const math::Vector2Dd& position);

			math::Vector2Dd camera_posision_;
