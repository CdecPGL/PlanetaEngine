...
namespace boost {
	namespace serialization {
		...
		void save(Archive& ar, const planeta_engine::core::GameDataComplexTypeInstance& game_data_element, unsigned int);
		template<class Archive>
...
namespace planeta_engine {
	namespace core {
		...
		class GameDataComplexTypeInstance {
		public:
			GameDataComplexTypeInstance() :complex_type_(nullptr) {};
		...
		private:
			const GameDataComplexType* complex_type_ = nullptr;
			mutable std::unordered_map<std::string, GameDataElement> elements_;
			GameDataComplexTypeInstance& operator=(const GameDataComplexTypeInstance&) = delete;
			GameDataComplexTypeInstance& operator=(GameDataComplexTypeInstance&&) = delete;
			//boostシリアライズのためのフレンド指定
			template<class Archive>
			friend void boost::serialization::save<Archive>(Archive& ar, const planeta_engine::core::GameDataComplexTypeInstance& game_data_element, unsigned int);
			template<class Archive>
			...
			//boostシリアライズ用
			void SetType_for_boost_serialize_(const GameDataComplexType& complex_type) { elements_.clear(); complex_type_ = &complex_type; }
		};
	}
}