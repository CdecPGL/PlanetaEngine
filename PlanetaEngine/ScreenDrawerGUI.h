...
namespace planeta {
	...
	public:
		...
		void DrawGraph(const util::RectAngle<int>& draw_area, const util::RectAngle<int>& draw_area_on_graph, bool reverse, const std::shared_ptr<resources::GraphResource>& graph_resource);
		...
		void DrawString(const Vector2Di& position, const Vector2Dd scale, const std::string& str, const planeta::Color& color, const planeta::Color& outline_color, const std::shared_ptr<resources::FontDefinitionResource>& font_definition_resource);
	private:
		core::Screen& screen_;
	};
}
