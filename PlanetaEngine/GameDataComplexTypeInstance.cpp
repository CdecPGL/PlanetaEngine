...
namespace planeta_engine {
	namespace core {
		...
		{
			if (complex_type_ == nullptr) { throw GameDataError("–³Œø‚ÈGameData•¡‡Œ^‚Å‚·B"); }
			...
			if (it == elements_.end()) {
				...
				if (complex_type().IsElementExist(element_id)) {
					...
					throw GameDataError(std::string("GameData•¡‡Œ^") + complex_type().type_id() + "‚É‘¶Ý‚µ‚È‚¢—v‘f" + element_id + "‚ÉƒAƒNƒZƒX‚µ‚æ‚¤‚Æ‚µ‚Ü‚µ‚½B");
				}
			}
			else {
				return it->second;
		...
		void GameDataComplexTypeInstance::SetElement(const std::string& element_id, const GameDataElement& element)
		{
			...
			auto it = elements_.find(element_id);
			if (it == elements_.end()) {
				if (!complex_type().IsElementExist(element_id)) { throw GameDataError(std::string("GameData•¡‡Œ^") + complex_type().type_id() + "‚É‘¶Ý‚µ‚È‚¢—v‘f" + element_id + "‚ÉƒAƒNƒZƒX‚µ‚æ‚¤‚Æ‚µ‚Ü‚µ‚½B"); }
				if (!complex_type().CheckElementType(element_id, element)) {
					throw GameDataError(std::string("GameData•¡‡Œ^") + complex_type().type_id() + "‚Ì—v‘f" + element_id + "(" + complex_type().GetElementType(element_id) + "Œ^)‚É" + element.type_id() + "‚Ì—v‘f‚ðŠi”[‚µ‚æ‚¤‚Æ‚µ‚Ü‚µ‚½‚ªAŒ^‚ªˆê’v‚µ‚Ä‚¢‚Ü‚¹‚ñB");
				}
