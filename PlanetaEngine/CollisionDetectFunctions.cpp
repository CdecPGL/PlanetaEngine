	...
	namespace core {
		namespace collision_detect_functions {
			...
			bool CollisionDetectCircleAndStraightLine(components::CircleColliderComponent& circle_collider, components::StraightLineColliderComponent& straightline_collider)
			{
				...
				auto line_vector = Vector2D<double>(std::cos(straightline_collider.GetCollisionGlobalRotationRad()), std::sin(straightline_collider.GetCollisionGlobalRotationRad()))*straightline_collider.GetCollisionScale()*straightline_collider.length();
