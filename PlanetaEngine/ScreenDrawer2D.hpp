...
namespace planeta {
	...
	public:
		...
		void DrawWire(const std::vector<Vector2Df>& positions, double width, const planeta::Color& color);
		/*多角形の描画(点座標、三角形ポリゴンインデックス、色)*/
		void DrawPolygon(const std::vector<Vector2Df>& positions, const std::vector<std::array<int, 3>>& indexes, const planeta::Color& color);
		/*円の描画*/
