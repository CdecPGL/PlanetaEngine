	...
	namespace core{
		...
		SceneManager::SceneStatus SceneManager::Process()
		{
			try {
				...
				debug::SystemLog::instance().LogError(std::string("Scene::Updateで無効なWeakPointerが参照されました。") + e.what(), __FUNCTION__);
				return SceneStatus::Error;
			}
		}
		...
		bool SceneManager::LoadNextScene(const std::string& scene_name){
			if (IsLoading() || IsTransitioning()){ 
				debug::SystemLog::instance().LogError(std::string("シーンの読み込みに失敗しました。シーンの読み込み中、または遷移中のため新たにシーン(") + scene_name + ")を読み込むことはできません。", __FUNCTION__);
				return false;
			} //すでに読み込み中か、遷移中のため、あらたに読み込みできない
			//シーン定義クラス作成
			_next_scene_setupper = _CreateSceneSetUpper(scene_name);
			if (_next_scene_setupper == nullptr) { 
				debug::SystemLog::instance().LogError(std::string("シーンの読み込みに失敗しました。指定されたシーン(") + scene_name + ")のセットアッパーが存在しません。", __FUNCTION__);
				return false; 
			} //シーン定義クラスが作成できなかった
			...
			if (_current_scene!=nullptr && _current_scene_setupper->CheckTransitionable(scene_name) == false) {
				debug::SystemLog::instance().LogError(std::string("シーンの読み込みに失敗しました。指定されたシーン(") + scene_name + ")は現在のシーンから遷移可能なシーンではありません。", __FUNCTION__);
				return false;
			}
			//リソース読み込み
			...
			if (core::ResourceManager::instance().PrepareResources(ntags) == false) {
				debug::SystemLog::instance().LogError(std::string("シーンの読み込みに失敗しました。指定されたシーン(") + scene_name + ")のリソース準備に失敗しました。", __FUNCTION__);
				return false; 
			} //リソースの準備ができなかった
			//準備完了
			_next_scene_id = scene_name;
			_is_next_scene_loaded = true;
			debug::SystemLog::instance().LogMessage(std::string("指定されたシーン(") + scene_name + ")を読み込みました。", __FUNCTION__);
			return true;
		}
		...
		bool SceneManager::TransitionScene(const utility::ParameterHolder& transition_parameters)
		{
			if (IsTransitionable()==false){
				debug::SystemLog::instance().LogError("シーン遷移予約に失敗しました。遷移中のため、新たに遷移処理をはじめることはできません。", __FUNCTION__);
				return false;
			} //遷移処理を始めることはできない
			//シーン遷移要求
			_request = _Request::Transition;
			_transition_parameters = transition_parameters;
			debug::SystemLog::instance().LogMessage("シーン遷移予約を行いました。", __FUNCTION__);
			return true;
		}
		...
		void SceneManager::_transition_proc()
		{
			...
			debug::SystemLog::instance().LogMessage(std::string("シーン(") + _next_scene_id + ")に遷移しました。", __FUNCTION__);
			_next_scene_id = "";
		}
